...
// These are the same generated pieces used by .open-next/worker.js, bound here
// individually so the generated dispatcher's request-time dynamic import of
// the Next handler is not reachable or bundled. The handler module has top-
// level initialization, so this static import completes in isolate scope.
...
import { createOpenNextFetchDispatcher } from "./src/lib/opennext-fetch-dispatcher";
...
const dispatchOpenNextRequest = createOpenNextFetchDispatcher({
...
export default {
  async fetch(request, env, ctx) {
    return requestDiagnostics.trace(request, () => dispatchOpenNextRequest(request, env, ctx));
  },
