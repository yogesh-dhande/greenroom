...
- [ ] **Auth for the evaluator's `run` command** — as shipped, the harness calls the Anthropic SDK directly and needs a real `ANTHROPIC_API_KEY` (~$2–10 per full run; `npm run smoke` needs no key). Claude is porting it to optionally run through your Claude Code subscription instead — this item will be updated with the outcome and exact command.
