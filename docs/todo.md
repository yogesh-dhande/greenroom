...
- [ ] **Run 4 manual checklist** — 22 items in `~/projects/killmysaas-evals/runs/2026-08-09T23-08-03/manual-checklist.md`. One timing trap: the SPK weekly-digest item only observably fires **Mondays 07:00–07:15 UTC** (D-039 schedule + 6-day cooldown) — check it in that window or it will look broken when it isn't.
- [ ] **Decide: evaluator DB reset before any run 5** (your call per D-046 — live evaluator accounts). Four runs of leftover fixture data now pollutes several areas (duplicate submissions/speakers/sessions were flagged as app defects again in run 4). The reset design preserves the evaluator sign-in accounts; say the word and it gets scripted and run against the deployed DB.
- [ ] **Decide: trigger eval run 5** — the run-4 fix batch (W25–W27 + two follow-ups) is now deployed with remote migrations applied; triage projects material gains (content-management alone has ~11 recoverable points). Runs only on your say-so.
