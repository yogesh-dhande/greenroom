...
## D-095: Destroying a filed scorecard requires explicit confirmation — **accepted** (2026-08-22)

**Decision:** Removing a round assignment that already has a scorecard filed against it is refused until the caller confirms it means to destroy the scorecard too. `unassignSubmission` takes a `confirmed` flag and, without it, returns the `SCORECARD_WOULD_BE_DELETED` code instead of deleting; the assignments UI answers that code with a dialog naming the reviewer, the submission, and the fact that the score leaves the round's results and export. Assignments with no score attached are unaffected and still remove in one click. The scorecard is still *deleted* on confirmation — this decision buys a warning, not retention.

**Rationale:** `round_scores.assignment_id` is `ON DELETE cascade`, so unassigning silently destroyed a reviewer's submitted scorecard — irreversibly, with nothing left to show it had existed. The 2026-08-18 evaluation lost two scorecards exactly this way and read the resulting empty results page as "scores don't save" (F3 in the gap report; root-caused 2026-08-22 from Workers Logs). The guard lives in the server action rather than only in the dialog because a server action is directly callable. Retaining scores as detached review history is the stronger answer and was considered, but a score currently has no identity apart from its assignment, so it needs a schema change; the warning removes the silent data loss at a fraction of the cost, and matches how the rest of the admin UI treats destructive actions. Revisit retention if review history is ever wanted as an audit trail (questions.md Q11, closed by this entry).
