...
**Rationale:** Production traces tied recurring 45–210-second outages to canceled, near-zero-CPU promises while sibling requests in other isolates completed. OpenNext 1.20.2's generated dispatcher dynamically imports the full Next handler inside every request; Cloudflare does not permit request-owned I/O promises to cross request contexts. The original preload-only D-082 implementation statically imported that handler but still imported the generated dispatcher. Inspection of the actual minified Wrangler bundle proved its per-request dynamic-import branch remained reachable, and the deployed preload-only version reproduced the signature on `/`: 50,081 ms wall, 23 ms CPU, `outcome=canceled`, no exception, while a sibling isolate returned 200. Copying the small generated routing shell and binding its final handler statically removes the branch rather than trying to warm it. Deferring cron-only modules still offsets the extra fetch-startup work. The statically dispatched production version then completed a 370-request, 15-minute mixed-persona soak and a separate 200-request concurrent burst without a stall; continued production observability remains warranted because fresh-isolate probes historically masked the long-lived failure.
