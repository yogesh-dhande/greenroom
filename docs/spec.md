...
- Admins can delete an unused form from its builder after confirmation. A form with any response or referenced by an onboarding task is protected; retire it by unpublishing it instead.
...
### 4. Review & decisions
- Routing: submissions pick tracks and reviewers own tracks — the default scoping for the review queue. Tracks stay editable after intake: an admin sets or corrects a submission's tracks from its record (reviewers never can — that would widen their own reach), so a submission that arrived with no track (e.g. via a form without a track question) can still be routed to a reviewer instead of being reachable by nobody. The walkthrough additionally shows evaluation plans that assign explicit submission sets to a reviewing committee (D-032, D-034), and the evaluator rubric tests exact per-reviewer assigned queues — covered by the evaluation-rounds work (D-031).
...
- Task reminders are a **weekly per-speaker digest** of everything still outstanding (sent Mondays 07:00 UTC), not a per-task cadence; it stops when the checklist is clear or the event starts, and admins can send it on demand (D-039). The digest's counterpart is a **one-time assignment notification** when new work lands on a speaker's checklist via any manual assign path; acceptance-created assignments send no separate notice — the acceptance email already carries the checklist (D-039).
...
- The composer's preview shows the **real event's** merge values — the template editor's inline preview included — and manual sends sign with the **sending admin's name**; a preview with placeholder dates or an `example.com` link is indistinguishable from broken mail (D-053). Admin stat cards (overview) count only their own event's data (D-045, D-053).
...
- Session length comes from duration presets or an exact custom minute count (whole minutes, 5–480). A session's length survives being sent back to the tray: unscheduling clears the day and room but keeps the time span as the session's remembered duration, so re-placing a short talk — by hand, drag, or Suggest a slot — doesn't reset it to the default.
- The public program is gated behind an explicit, reversible **publish** action per event; unpublished events show a "program coming soon" state on all public/embed program surfaces while the CFP form keeps its own open/close state (D-056). New events start unpublished. Because only approved content goes public (D-072), publishing tells the admin which scheduled sessions will be held back awaiting content sign-off — in the confirmation before the fact, and as a persistent note on the publish card for as long as the condition holds — so the public program never silently disagrees with the scheduled count.
...
- Public, mobile-friendly **speaker gallery** and **schedule**, embeddable on an external website. The admin embed builder offers List of Sessions, List of Speakers, Agenda, Schedule Itinerary, and Speaker Gallery widgets as script/basic HTML, iframe, JSON, XML, or iCal output. Configuration is stateless in the generated URL and supports track/approved-content filters, optional card fields, brand colors, and custom CSS (D-080).
...
- **Content-management depth** (D-031): file re-upload creates versions (latest marked, older accessible); comments on uploaded files (author + timestamp, cross-role); a session **content approval status** (draft/in review/approved) separate from scheduling status — only approved content appears on public surfaces, existing content grandfathered as approved (D-072); **abstract revision history** — every edit to a session's abstract records who/when/prior value, viewable on the session with a per-entry restore that is itself recorded, history append-only (D-071); a central files library across sessions and profile uploads, showing each file's session(s), with selectable ZIP export of current files grouped by speaker, session, or no folders (D-079).
- **Public program depth** (D-031): keyword search (titles + speaker names) and track filters on the sessions/schedule views; speaker directory search; session and speaker detail views; personal itinerary (star sessions, persists, `.ics` export). Session surfaces (cards, detail view, feeds) carry speaker **name, title, and company** and a short description snippet — the program's `.ics` feed includes each session's speaker names, and a published program always serves a valid feed (an empty calendar before anything is scheduled, never a 404); a speaker card's talk list labels not-yet-scheduled talks ("time to be announced") so the gallery never silently disagrees with the schedule. Wrong URLs get a branded 404, never the framework default: an unknown path under a live event keeps that event's own header and nav with a link back to its program.
...
- Generalized CMS tooling and authenticated write API beyond what the UI needs (public read feeds are included).
- A **small** useful agentic feature that reduces real admin work (narrow assistant, not a chatbot).

## Org-level speaker CRM (optional extra-credit area; D-077, supersedes D-059)

- A cross-event **contact directory** at organization level, outside any single event: every speaker across events plus directly-added contacts, in a searchable table (name, email, title, company, tags, connected-event count) with attribute filters (company, tag) that compose AND-style and clear in one action.
- **Contact profiles**: identity fields, org-level internal notes that persist, free-form tags, a list of connected events/sessions, and an activity feed (logged emails, pipeline stage moves, notes).
- **Org-level CSV import** (same column rules as the event roster import, deduplicating by email) and manual contact creation. Duplicates are checked before creation, not just flagged after: an address already in the directory is rejected outright (email is the identity key, D-051), while a shared name under a different email still creates the contact with a note pointing at the possible duplicate (D-059).
- **Saved segments**: a filtered directory view saves under a name as a dynamic segment and reopens with its current matches.
- **Sourcing pipeline**: a kanban board with stages Identified, Contacted, Interested, Confirmed, Declined; contacts enroll with an optional score and rationale; cards move between stages via an explicit control and stay put across reload; card detail carries internal notes and a timestamped stage history. A table view of the same cards (toggled alongside the board, optionally filtered to one stage) sorts by last stage-touch, stalest first, so neglected prospects surface.
- **Add to event**: from a profile or pipeline card, an event picker attaches the contact to that event's roster with profile data carried over without re-entry (identity is global, D-051).
- **Bulk outreach**: select directory contacts and email them with the merge-tag composer; sends land in the communications log.
- **CRM overview**: org-wide KPIs (total contacts, events, returning speakers, pipeline stage counts) and a top-companies widget that links into the filtered directory.
- Excluded: duplicate-record **merge** (D-065 — the duplicate flag and side-by-side links remain the guard) and custom-field builders (tags only). Event slugs `directory`, `pipeline`, `crm` are reserved.

## Out of scope

- Payments; **Accelevents integration** (organizer explicitly dropped it); marketing automation beyond the CRM's bulk outreach; contact-record merge tooling (D-065); content transcription/repurposing; full Sessionboard CMS recreation; AI-assisted evaluation; large agentic systems; multilingual forms; video links in calendar invites; pixel-level Sessionboard fidelity; attendee registration/ticketing.
